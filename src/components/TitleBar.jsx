...
const TitleBar = () => {
    ...
    return (
        <>
            <div data-tauri-drag-region className={`h-8 flex-none flex items-center justify-between px-2 select-none z-50 border-b ${bgClass} ${borderClass}`}>
                <div data-tauri-drag-region className="flex items-center gap-2">
                    <div className="w-5 h-5 rounded bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center">
                        <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                        </svg>
                    </div>
                    <span className={`text-xs font-medium ${textClass} whitespace-nowrap`}>Restaurant Billing</span>
                </div>
                        ...
                        <div className="flex items-center gap-1">
                            {menuItems.map((item) => (
                                <NavLink
                                    ...
                                    to={item.path}
                                    className={({ isActive }) =>
                                        `px-3 py-1 rounded text-[11px] font-medium transition-colors whitespace-nowrap ${isActive
                ...
                <div className="flex items-center gap-2">
                    ...
                    {!isAuthPage && (
                        <div className="flex items-center gap-1">
                            <button onClick={toggleTheme} className={`w-8 h-8 flex items-center justify-center rounded transition-colors ${hoverClass}`} title="Toggle Dark/Light Mode">
                                {theme === 'dark' ? (
                            ...
                            <div className="relative" ref={notificationRef}>
                                <button onClick={() => setShowNotifications(!showNotifications)} className={`w-8 h-8 flex items-center justify-center rounded transition-colors relative ${hoverClass}`} title="Notifications">
                                    ...
                                    <div className="absolute top-full right-0 mt-1 w-64 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-2 z-50">
                                        ...
                                        <div className="max-h-64 overflow-y-auto">
                                            <div className="px-3 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer">
                                                <p className="text-xs text-gray-800 dark:text-gray-200">New order received</p>
                            ...
                            <div className="relative" ref={profileRef}>
                                <button onClick={() => setShowProfile(!showProfile)} className={`w-8 h-8 flex items-center justify-center rounded transition-colors ${hoverClass}`} title="Profile">
                                ...
                                {showProfile && (
                                    <div className="absolute top-full right-0 mt-1 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-2 z-50">
                                        <div className="px-3 py-2 border-b border-gray-200 dark:border-gray-700">
                                        ...
                                        <button className="w-full px-3 py-2 text-left text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700">My Profile</button>
                                        <button className="w-full px-3 py-2 text-left text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700">Settings</button>
                                        <div className="border-t border-gray-200 dark:border-gray-700 my-1"></div>
                                        <button onClick={handleLogout} className="w-full px-3 py-2 text-left text-xs text-red-600 dark:text-red-400 hover:bg-gray-100 dark:hover:bg-gray-700">Logout</button>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="flex items-center">
                        <button onClick={handleMinimize} className={`w-10 h-8 flex items-center justify-center transition-colors ${hoverClass}`} title="Minimize">
                            <svg className={`w-4 h-4 ${iconClass}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 12H4" />
                            </svg>
                        </button>
                        <button onClick={handleMaximize} className={`w-10 h-8 flex items-center justify-center transition-colors ${hoverClass}`} title="Maximize/Restore">
                            <svg className={`w-3.5 h-3.5 ${iconClass}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4h16v16H4z" />
                            </svg>
                        </button>
                        <button onClick={handleCloseClick} className={`w-10 h-8 flex items-center justify-center hover:bg-red-600 transition-colors`} title="Close">
                            <svg className={`w-4 h-4 ${iconClass}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
